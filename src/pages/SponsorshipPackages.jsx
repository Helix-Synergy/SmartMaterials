...
import event from "../assets/Images1/Eventpartners.webp"
    ...
    <div className="w-full  mx-auto justify-center items-center text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mx-4 md:mx-12">
        {sponsorshipPackages.map((pkg) => (
          <div
            key={pkg.type}
            className="border border-gray-200 rounded-xl shadow-md p-4 bg-white items-center flex flex-col justify-between"
          >
              ...
              <ul className="list-disc list-inside text-gray-700 space-y-2">
                {pkg.benefits.map((benefit, index) => (
                  <div className="flex flex-row gap-2 items-center" key={index}>
                    ...
                    <li className="text-[15px] list-none">{benefit}</li>
                  </div>
                ))}
              </ul>
            </div>
      ...
      <div className="my-12 mx-4 md:mx-32">
        {last_item.map((pkg) => (
          ...
          >
            <div>
              {/* Badge and Exhibitor text side-by-side */}
                  ...
                  <ul key={groupIndex} className="space-y-2">
                    {group.map((benefit, index) => (
                      ...
                      >
                        <Sparkles
                          size={14}
                          className="text-one mt-1 fill-one"
                        />
                        <span>{benefit}</span>
                      </li>
                    ))}
